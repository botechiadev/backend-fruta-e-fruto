...
class TicketsController {
    private ticketFilePath: string;
    private ticketData: IRecipeDB[];

    constructor() {
        this.ticketFilePath = path.join(__dirname, '../../json/jsonTicket.json');
        this.ticketData = this.loadTicket();
    }

    private loadTicket(): IRecipeDB[] {
        try {
            const fileContent = fs.readFileSync(this.ticketFilePath, 'utf-8');
    ...
    private saveticket(): void {
        try {
            fs.writeFileSync(this.ticketFilePath, JSON.stringify(this.ticketData));
        } catch (error) {
            console.error(error);
        }
    }

    public getAllticket = async (req: Request, res: Response): Promise<void> => {
    ...
    public editRecipe = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (recipeIndex === -1) {
                ...
                this.ticketData[recipeIndex] = { ...this.ticketData[recipeIndex], ...req.body };
